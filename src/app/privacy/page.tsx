        ...
        <div className="prose prose-amber max-w-none space-y-8 text-amber-900/80 leading-relaxed">
          ...
          {Array.isArray(mainContent.sections) && mainContent.sections.map((s: any, i: number) => (
            <section key={i} className="space-y-6">
              <h2 className="font-serif text-3xl text-amber-950 mt-8 mb-4 border-b border-amber-900/5 pb-2">{s.title}</h2>
              <div className="space-y-4">
                {s.content.split(/\n|(?=\s*•)|(?=\s*\d+\.\s+)/).filter((p: string) => p.trim()).map((point: string, j: number) => {
                  const cleanedPoint = point.replace(/^\s*[•\d+\.]\s*/, '').trim();
                  if (!cleanedPoint) return null;
                  return (
                    <div key={j} className="flex gap-4 items-start group">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-900/40 mt-[0.6rem] shrink-0 group-hover:bg-amber-900/80 transition-colors" />
                      <p className="m-0 text-amber-950/80">{cleanedPoint}</p>
                    </div>
                  );
                })}
              </div>
