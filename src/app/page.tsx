...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [productsRes, contentRes, categoriesRes, offersRes] = await Promise.all([
  ...
  return (
    <div className="bg-[#f9f7f2] min-h-screen">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {product.originalPrice && (
                          <p className="text-[10px] text-amber-900/40 line-through">₹{product.originalPrice}</p>
                        )}
                      </div>
                    </div>
                  </Link>
      ...
      {offers.map((offer) => {
        const offerProducts = featuredProducts.filter(p => p.offerId === offer.id);
        const allOfferProducts = [...recentProducts, ...featuredProducts].filter(p => p.offerId === offer.id);
        // Deduplicate
        const uniqueOfferProducts = Array.from(new Map(allOfferProducts.map(item => [item.id, item])).values());
        
        if (uniqueOfferProducts.length === 0) return null;
            ...
            <div className="max-w-7xl mx-auto px-6">
              <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
                <div>
                  ...
                  <h2 className="font-serif-alt text-6xl text-white italic">{offer.name}</h2>
                  <p className="text-amber-50/60 mt-6 max-w-xl text-lg">{offer.description}</p>
                </div>
                <Link href="/catalog" className="text-[10px] uppercase tracking-widest text-white font-bold border-b border-white/20 pb-1 hover:border-white transition-all">
                  Shop the Offer
                </Link>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {uniqueOfferProducts.slice(0, 4).map((product, i) => (
                  <motion.div
                    key={product.id}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <Link href={`/product/${product.id}`} className="block h-full">
                      ...
                      <div className="space-y-2">
                        <h3 className="font-serif text-xl text-white group-hover:text-amber-200 transition-colors">{product.name}</h3>
                        <div className="flex items-center gap-3">
                          <p className="text-amber-200 font-medium italic">₹{product.price}</p>
                          {(offer.discount || product.originalPrice) && (
                            <p className="text-[10px] text-white/30 line-through">₹{product.originalPrice || Math.round(product.price * 1.2)}</p>
                          )}
                        </div>
                      </div>
                    </Link>
                        ...
                        <div className="mt-6 p-4 bg-white flex-1 border-t-0 border border-amber-900/5 hover:bg-amber-50/30 transition-colors">
                          ...
                          <div className="flex justify-between items-center mt-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {product.originalPrice && (
                                <p className="text-[10px] text-amber-900/30 line-through">₹{product.originalPrice}</p>
                              )}
                            </div>
                          </div>
                        </div>
